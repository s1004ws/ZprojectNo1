package com.project.webApp.admin;

public class AdminVO {
	
	private int num;
	private String cafename;
	private String cafemainmenu;
	private String cafephone;
	private String cafeaddr;
	private String latitude;
	private String longitude;
	private String cafefavor;
	private String cafepic1;
	private String cafepic2;
	private String cafepic3;
	private int cafestar;
	private String cafeparking;
	private String cafeholi;
	private String cafetime;
	private String cafeshutdown;
	private String cafedelivery;
	private String cafewritedate;
	private String cafelunch;
	private String cafedinner;
	private String cafelate;
	private String cafealcohol;
	
	///
	public AdminVO(){}

	public int getNum() {
		return num;
	}

	public void setNum(int num) {
		this.num = num;
	}

	public String getCafename() {
		return cafename;
	}

	public void setCafename(String cafename) {
		this.cafename = cafename;
	}

	public String getCafemainmenu() {
		return cafemainmenu;
	}

	public void setCafemainmenu(String cafemainmenu) {
		this.cafemainmenu = cafemainmenu;
	}

	public String getCafephone() {
		return cafephone;
	}

	public void setCafephone(String cafephone) {
		this.cafephone = cafephone;
	}

	public String getCafeaddr() {
		return cafeaddr;
	}

	public void setCafeaddr(String cafeaddr) {
		this.cafeaddr = cafeaddr;
	}

	public String getLatitude() {
		return latitude;
	}

	public void setLatitude(String latitude) {
		this.latitude = latitude;
	}

	public String getLongitude() {
		return longitude;
	}

	public void setLongitude(String longitude) {
		this.longitude = longitude;
	}

	public String getCafefavor() {
		return cafefavor;
	}

	public void setCafefavor(String cafefavor) {
		this.cafefavor = cafefavor;
	}

	public String getCafepic1() {
		return cafepic1;
	}

	public void setCafepic1(String cafepic1) {
		this.cafepic1 = cafepic1;
	}

	public String getCafepic2() {
		return cafepic2;
	}

	public void setCafepic2(String cafepic2) {
		this.cafepic2 = cafepic2;
	}

	public String getCafepic3() {
		return cafepic3;
	}

	public void setCafepic3(String cafepic3) {
		this.cafepic3 = cafepic3;
	}

	public int getCafestar() {
		return cafestar;
	}

	public void setCafestar(int cafestar) {
		this.cafestar = cafestar;
	}

	public String getCafeparking() {
		return cafeparking;
	}

	public void setCafeparking(String cafeparking) {
		this.cafeparking = cafeparking;
	}

	public String getCafeholi() {
		return cafeholi;
	}

	public void setCafeholi(String cafeholi) {
		this.cafeholi = cafeholi;
	}

	public String getCafetime() {
		return cafetime;
	}

	public void setCafetime(String cafetime) {
		this.cafetime = cafetime;
	}

	public String getCafeshutdown() {
		return cafeshutdown;
	}

	public void setCafeshutdown(String cafeshutdown) {
		this.cafeshutdown = cafeshutdown;
	}

	public String getCafedelivery() {
		return cafedelivery;
	}

	public void setCafedelivery(String cafedelivery) {
		this.cafedelivery = cafedelivery;
	}

	public String getCafewritedate() {
		return cafewritedate;
	}

	public void setCafewritedate(String cafewritedate) {
		this.cafewritedate = cafewritedate;
	}

	public String getCafelunch() {
		return cafelunch;
	}

	public void setCafelunch(String cafelunch) {
		this.cafelunch = cafelunch;
	}

	public String getCafedinner() {
		return cafedinner;
	}

	public void setCafedinner(String cafedinner) {
		this.cafedinner = cafedinner;
	}

	public String getCafelate() {
		return cafelate;
	}

	public void setCafelate(String cafelate) {
		this.cafelate = cafelate;
	}

	public String getCafealcohol() {
		return cafealcohol;
	}

	public void setCafealcohol(String cafealcohol) {
		this.cafealcohol = cafealcohol;
	}
	
	
	
	///
	
	
	
}
